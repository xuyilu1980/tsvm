...
function [w b new_alpha] = train_linear_transductive_svm1(X,y,C1,C2,w0,b0)
s = -0.20;
...
for it=1:5 
    ...
    UB=[1   C1*ones(1,npos)   zeros(1,nneg)     C2-beta(1,1:U) beta(1,U+1:2*U)]; 

    yyy=ones(1,length(yy));
    [new_alpha,faval]=mlcv_quadprog(XX',yyy,kernel,-f,Aeq,beq,LB,UB);%Generalized SMO algorithm.

    
    %[new_alpha,faval]=quadprog(HH,-f,[],[],Aeq,beq,LB,UB);
    faval
