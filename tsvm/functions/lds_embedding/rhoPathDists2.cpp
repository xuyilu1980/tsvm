...
void calcPathDists0( double* const E2,
		     ...
		     const int* const subset )  //相比calcPathDistRho少了rho变量，rho=0的话就回到了欧氏距离
{
  // === variables
		       ...
		       const double rho )
{
  ...
  if( k == 0 ) {
    for( j = 0; j < n*n; ++j ) {
      Dr[j] = exp( rho * sqrt(D2[j]) ) - 1.0;//像第6页,算法1的第1点
    }
  } else {
    register int p;
    for( p = 0; p < k*n; ++p ) {
      Dr[p] = exp( rho * sqrt(D2[p]) ) - 1.0; //这里算是论文第6页算法中的第1条
    }
  }
  // === compute
  er = E2;
  for( ii = 0; ii < l; ++ii ) {
    ...
    // --- iteratively update squared distances，下面应该就是Dijkstra
    while( nofOpen > 0 ) {
    ...
    for( j = 0; j < n; ++j ) {
      register const double t = log( 1 + er[j] ) / rho;//这个就是第6页算法1的第3点
      er[j] = t * t;
    }
    // --- next element of subset
    er += n;
